...
void APuzzleGameCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindTouch(IE_Pressed, this, &APuzzleGameCharacter::TouchStarted);
	PlayerInputComponent->BindTouch(IE_Released, this, &APuzzleGameCharacter::TouchStopped);
...
{
	// If PuzzleGame is added to a project via 'Add Feature' in the Unreal Editor the dependency on HeadMountedDisplay in PuzzleGame.Build.cs is not automatically propagated
...
void APuzzleGameCharacter::TouchStarted(ETouchIndex::Type FingerIndex, FVector Location)
{
		Jump();
}

void APuzzleGameCharacter::TouchStopped(ETouchIndex::Type FingerIndex, FVector Location)
{
		StopJumping();
}
